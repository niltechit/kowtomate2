...
namespace CutOutWiz.Services.StorageService
{
    ...
    {                         
        ...
        /// <returns></returns>
        public async Task<Response<List<Node>>> GetDirectories(AwsCredentialsModel awsCredentials, string prefix, string delimiter = "/")
        {
            var methodResponse = new Response<List<Node>>();
            ...
            try
            {
                ...
                using (var s3 = new AmazonS3Client(awsCredentials.AccessKey, awsCredentials.SecretAccessKey, RegionEndpoint.USEast1))
                {
                    ...
                    foreach (var d in DirectoryListWithoutCompleted)
                    {
                        ...
                        directories.Add(new Node(d, folderName, true));
                    }
                }
        ...
        /// <returns></returns>
        public async Task<Response<List<DriveImage>>> GetFilesByPath(AwsCredentialsModel awsCredentials, string prefix, bool loadRawImages = true)
        {
            var methodResponse = new Response<List<DriveImage>>();
            ...
            try
            {
                ...
                List<DriveImage> Files = new List<DriveImage>();
                        ...
                        if (fileName != "")
                        {
                            foreach (var imgInfoForToProcess in imageInfoForToProcess)
                            {
                                ...
                                else
                                {
                                    ...
                                    var image = new DriveImage(prefix, fileName, imageInfo.Key, fileSize, imageInfo.LastModified);
