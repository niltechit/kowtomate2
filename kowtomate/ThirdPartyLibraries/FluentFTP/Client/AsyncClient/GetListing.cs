	...
	public partial class AsyncFtpClient {
		...
		protected async Task<FtpListItem[]> GetListingRecursive(string path, FtpListOption options, CancellationToken token) {
			...
			while (stack.Count > 0) {
				...
				// list it
				if (options.HasFlag(FtpListOption.NoPath)) {
					await SetWorkingDirectory(currentPath);
					items = await GetListing(null, options, token);
				}
				else {
					items = await GetListing(currentPath, options, token);
				}
